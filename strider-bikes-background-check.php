...
class Strider_Bikes_Background_Check{
    ...
    function __construct(){
        ...
        add_action('wp', array($this, 'check_if_course_passed'));
    }
   ...
   // add meta box to set page as locked until background check is complete
    ...
    function sb_bg_check_status_shortcode(){
        ...
        $congrats = '<div class="container-fluid">';
        if(!empty($passedCourses)){
            foreach($passedCourses as $pc){
                $congrats .= '<h2> Congrats you have passed: </h2>';
                $congrats += '<p>' . get_the_title($pc) . '</p>';
            }
            $congrats .= '</div>';
        } else {
            $congrats .= '<p>no courses passed</p> </div>';
        }
